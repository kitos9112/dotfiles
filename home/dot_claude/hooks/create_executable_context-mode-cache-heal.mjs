...
try {
  ...
  for (const [k, es] of Object.entries(ip.plugins || {})) {
    ...
    for (const e of es) {
      ...
      if (existsSync(p)) {
        ...
        try {
          ...
          if (existsSync(nhPath)) {
            ...
            const fn =
              mod.normalizeHooksJsonOnly || mod.normalizeHooksOnStartup;
            if (fn)
              fn({
                pluginRoot: p,
      ...
      const dirs = readdirSync(parent).filter(
        (d) => /^\d+\.\d+/.test(d) && statSync(join(parent, d)).isDirectory(),
      );
      ...
      try {
        symlinkSync(
          join(parent, dirs[dirs.length - 1]),
          p,
