...
GET_SECRET_TEMPLATE="${REPO_ROOT}/home/private_dot_local/private_bin/executable_ipa-get-secret.tmpl"
...
GET_SECRET_SCRIPT="${TMP_ROOT}/ipa-get-secret"
...
assert_status 0 "ipa-get-secret --help must succeed"
assert_contains 'Usage: ipa-get-secret' "${STDOUT_FILE}" \
	"--help must print usage on stdout"
pass "ipa-get-secret --help exits 0 and prints usage"

run_capture "${GET_SECRET_SCRIPT}"
assert_status 2 "a missing vault key must be a usage error"
pass "ipa-get-secret without a vault key exits 2"
...
pass "ipa-get-secret reports a missing 'ipa' client as exit 127"
...
pass "ipa-get-secret preflights the Kerberos ticket with an actionable error"
...
pass "ipa-get-secret decodes the secret byte for byte and forwards options"
